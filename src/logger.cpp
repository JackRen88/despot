...
EvalLog::EvalLog(string log_file) : log_file_(log_file) {
...
int EvalLog::GetNumCompletedRuns(string instance) const {
  for (int i = 0; i < runned_instances.size(); i++) {
    if (runned_instances[i] == instance) return num_of_completed_runs[i];
  }
  return 0;
}
...
void EvalLog::SetInitialBudget(string instance) {
  ...
  if (GetNumRemainingRuns() != 0 && GetNumRemainingRuns(instance) != 0) {
    ...
    if (curr_inst_budget < 0) curr_inst_budget = 0;
    if (curr_inst_budget > 18 * 60) curr_inst_budget = 18 * 60;
  }
}

double EvalLog::GetRemainingBudget(string instance) const {
  return curr_inst_budget - (get_time_second() - EvalLog::curr_inst_start_time);
}
...
Logger::Logger(DSPOMDP* model, Belief* belief, Solver* solver, World* world,
               string world_type, clock_t start_clockt, ostream* out,
...
double Logger::AverageUndiscountedRoundReward() const {
  ...
  return undiscounted_round_rewards_.size() > 0
             ? (sum / undiscounted_round_rewards_.size())
             : 0.0;
}
...
double Logger::AverageDiscountedRoundReward() const {
  ...
  return discounted_round_rewards_.size() > 0
             ? (sum / discounted_round_rewards_.size())
             : 0.0;
}
...
double Logger::EndRound() {
  if (!Globals::config.silence && out_) {
    *out_ << "Total discounted reward = " << total_discounted_reward_ << endl
          << "Total undiscounted reward = " << total_undiscounted_reward_
          << endl;
  }
...
bool Logger::SummarizeStep(int step, int round, bool terminal, ACT_TYPE action,
  ...
  *out_ << "-----------------------------------Round " << round << " Step "
  ...
  if (state_ != NULL) {
    if (!Globals::config.silence && out_)
      *out_ << "- ObsProb = " << model_->ObsProb(obs, *state_, action) << endl;
  }
  ...
  if (world_type_ == "pomdp")
    ...
    if (reward_ >
      ...
      reward_ = 0;
      logd << "[Logger::SummarizeStep] Reward function has not been defined in "
              "DSPOMDP model"
           << endl;
    }
  }
  ...
  if (terminal) {
    ...
    if (!Globals::config.silence && out_) *out_ << endl;
    step_++;
    return true;
  }
  *out_ << endl;
  ...
  if (target_finish_time_ != -1) {
    if (step_time < 0.99 * EvalLog::allocated_time) {
      if (EvalLog::plan_time_ratio < 1.0) EvalLog::plan_time_ratio += 0.01;
      if (EvalLog::plan_time_ratio > 1.0) EvalLog::plan_time_ratio = 1.0;
    ...
    EvalLog::curr_inst_remaining_budget =
        target_finish_time_ - get_time_second();
    ...
    if (EvalLog::curr_inst_remaining_steps <= 0) {
      ...
      if (EvalLog::allocated_time > 5.0) EvalLog::allocated_time = 5.0;
    }

    Globals::config.time_per_move =
        EvalLog::plan_time_ratio * EvalLog::allocated_time;
  }
  logi << "[main] Time per move set to " << Globals::config.time_per_move
       << endl;
  logi << "[main] Plan time ratio set to " << EvalLog::plan_time_ratio << endl;
