...
option::Descriptor *BuildUsage(string lower_bounds_str,
  ...
  static option::Descriptor usage[] = {
      ...
      {E_PARAMS_FILE, 0, "m", "model-params", option::Arg::Required,
       ...
       "any."},
      {E_WORLD_FILE, 0, "w", "world-params", option::Arg::Required,
      ...
      {E_DEPTH, 0, "d", "depth", option::Arg::Required,
       "-d <arg>  \t--depth <arg>  \tMaximum depth of search tree (default "
       "90)."},
      ...
      {E_TIMEOUT, 0, "t", "timeout", option::Arg::Required,
       "-t <arg>  \t--timeout <arg>  \tSearch time per move, in seconds "
       "(default "
       "1)."},
      ...
      {E_SIM_LEN, 0, "s", "simlen", option::Arg::Required,
       "-s <arg>  \t--simlen <arg>  \tNumber of steps to simulate. (default "
       "90; 0 "
       "= infinite)."},
...
PlannerBase::PlannerBase(string lower_bounds_str, string base_lower_bounds_str,
                         string upper_bounds_str,
                         string base_upper_bounds_str) {
  usage = BuildUsage(lower_bounds_str, base_lower_bounds_str, upper_bounds_str,
                     base_upper_bounds_str);
}
...
World *PlannerBase::InitializeWorld(std::string &world_type, DSPOMDP *model,
  ...
  if (world_type == "pomdp")
    ...
    cerr << "Unsupported world type (pomdp by default). To support custom "
            "world, implement "
         << __FUNCTION__ << endl;
    exit(-1);
  }
...
option::Option *PlannerBase::InitializeParamers(
    int argc, char *argv[], std::string &solver_type, bool &search_solver,
    int &num_runs, std::string &world_type, std::string &belief_type,
    int &time_limit) {
...
void PlannerBase::OptionParse(option::Option *options, int &num_runs,
  ...
  if (options[E_WORLD]) world_type = options[E_WORLD].arg;
...
void PlannerBase::InitializeLogger(Logger *&logger, option::Option *options,
                                   DSPOMDP *model, Belief *belief,
                                   Solver *solver, int num_runs,
                                   clock_t main_clock_start, World *world,
                                   string world_type, int time_limit,
                                   string solver_type) {
  if (time_limit != -1) {
    logger =
        new Logger(model, belief, solver, world, world_type, main_clock_start,
                   &cout, EvalLog::curr_inst_start_time + time_limit,
