...
ScenarioLowerBound* DESPOT::lower_bound() const { return lower_bound_; }

ScenarioUpperBound* DESPOT::upper_bound() const { return upper_bound_; }
...
VNode* DESPOT::Trial(VNode* root, RandomStreams& streams,
                     ...
                     History& history, SearchStatistics* statistics) {
  VNode* cur = root;

  int hist_size = history.Size();

  do {
    if (statistics != NULL && cur->depth() > statistics->longest_trial_length) {
...
VNode* DESPOT::ConstructTree(vector<State*>& particles, RandomStreams& streams,
                             ...
                             double timeout, SearchStatistics* statistics) {
  if (statistics != NULL) {
  ...
  logd << "[DESPOT::ConstructTree] START - Initializing lower and upper bounds "
          "at the root node.";
  InitBounds(root, lower_bound, upper_bound, streams, history);
  logd << "[DESPOT::ConstructTree] END - Initializing lower and upper bounds "
          "at the root node.";
...
void DESPOT::Compare() {
  ...
  VNode* root =
      ConstructTree(particles, streams, lower_bound_, upper_bound_, model_,
                    history_, Globals::config.time_per_move, &statistics);
...
ValuedAction DESPOT::Search() {
  ...
  logi << "[DESPOT::Search] Time for deleting tree: "
       << (get_time_second() - start) << "s" << endl;
  logi << "[DESPOT::Search] Search statistics:" << endl << statistics_ << endl;
...
double DESPOT::CheckDESPOT(const VNode* vnode, double regularized_value) {
  ...
  do {
    double start = clock();
    VNode* cur =
        Trial(root, streams, lower_bound_, upper_bound_, model_, history_);
    num_trials++;
    used_time += double(clock() - start) / CLOCKS_PER_SEC;
    ...
    if (double(num_trials - prev_num) > 0.05 * prev_num) {
      ...
      cout << "# trials = " << num_trials << "; target = " << regularized_value
           << ", current = " << pruned_value << ", l = " << root->lower_bound()
           << ", u = " << root->upper_bound() << "; time = " << used_time
           << endl;
  ...
  cout << "DESPOT: # trials = " << num_trials
       << "; target = " << regularized_value << ", current = " << pruned_value
...
double DESPOT::CheckDESPOTSTAR(const VNode* vnode, double regularized_value) {
  ...
  cout << "DESPOT: # trials = " << num_trials
       << "; target = " << regularized_value
       << ", current = " << root->lower_bound()
       << ", l = " << root->lower_bound() << ", u = " << root->upper_bound()
       << "; time = " << used_time << endl;
  cout << "--------------------------------------------------------------------"
          "------------"
...
VNode* DESPOT::Prune(VNode* vnode, ACT_TYPE& pruned_action,
                     double& pruned_value) {
  vector<State*> empty;
  VNode* pruned_v = new VNode(empty, vnode->depth(), NULL, vnode->edge());
...
double DESPOT::WEU(VNode* vnode) { return WEU(vnode, Globals::config.xi); }
...
void DESPOT::Expand(QNode* qnode, ScenarioLowerBound* lb,
  ...
  step_reward =
      Globals::Discount(parent->depth()) * step_reward -
      Globals::config
          .pruning_constant;  // pruning_constant is used for regularization
...
ValuedAction DESPOT::Evaluate(VNode* root, vector<State*>& particles,
                              RandomStreams& streams, POMCPPrior* prior,
                              const DSPOMDP* model) {
  double value = 0;
  ...
  for (int i = 0; i < particles.size(); i++) {
    ...
    while (!streams.Exhausted()) {
      ACT_TYPE action =
          (cur != NULL) ? OptimalAction(cur).action : prior->GetAction(*copy);
