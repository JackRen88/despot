...
ParticleBelief::ParticleBelief(vector<State*> particles, const DSPOMDP* model,
  ...
  if (split) {
    ...
    while (2 * num_particles_ < 5000) num_particles_ *= 2;
    if (particles_.size() < num_particles_) {
      ...
      for (int i = 0; i < particles_.size(); i++) model_->Free(particles_[i]);
...
const vector<State*>& ParticleBelief::particles() const { return particles_; }
...
void ParticleBelief::Update(ACT_TYPE action, OBS_TYPE obs) {
  ...
  for (int i = 0; i < particles_.size(); i++) {
    ...
    bool terminal =
        model_->Step(*particle, Random::RANDOM.NextDouble(), action, reward, o);
    double prob = model_->ObsProb(obs, *particle, action);

    if (!terminal && prob) {  // Terminal state is not required to be explicitly
                              // represented and may not have any observation
  ...
  if (particles_.size() == 0) {
    ...
    if (prior_ != NULL) {
      logw << "Resampling by drawing random particles from prior which are "
              "consistent with history"
           << endl;
      particles_ = Resample(num_particles_, *prior_, history_);
    } else {
      logw << "Resampling by searching initial particles which are consistent "
              "with history"
           << endl;
      particles_ =
    ...
    if (particles_.size() == 0 && state_indexer_ != NULL) {
      logw << "Resampling by searching states consistent with last (action, "
              "observation) pair"
           << endl;
      particles_ =
    ...
    // Update total weight so that effective number of particles are computed
    // correctly
    total_weight = 0;
    for (int i = 0; i < particles_.size(); i++) {
      State* particle = particles_[i];
...
vector<State*> ParticleBelief::Resample(int num, const vector<State*>& belief,
  ...
  while (count < num && trial < 200 * num) {
    ...
    for (int i = hstart; i < history.Size(); i++) {
      model->Step(*particle, Random::RANDOM.NextDouble(), history.Action(i),
                  reward, obs);

      double prob =
          model->ObsProb(history.Observation(i), *particle, history.Action(i));
      if (prob > 0) {
        log_wgt += log(prob);
      } else {
        model->Free(particle);
...
vector<State*> ParticleBelief::Resample(int num, const Belief& belief,
  ...
  while (count < num || trial < 200 * num) {
    ...
    for (int i = hstart; i < history.Size(); i++) {
      ...
      double prob = belief.model_->ObsProb(history.Observation(i), *particle,
                                           history.Action(i));
      if (prob > 0) {
        log_wgt += log(prob);
      } else {
