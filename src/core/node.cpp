...
VNode::VNode(vector<State*>& particles, int depth, QNode* parent, OBS_TYPE edge)
...
VNode::~VNode() {
  ...
  if (belief_ != NULL) delete belief_;
}
...
void VNode::depth(int d) { depth_ = d; }
...
OBS_TYPE VNode::edge() { return edge_; }

double VNode::Weight() const { return State::Weight(particles_); }

const vector<QNode*>& VNode::children() const { return children_; }

vector<QNode*>& VNode::children() { return children_; }

const QNode* VNode::Child(ACT_TYPE action) const { return children_[action]; }

QNode* VNode::Child(ACT_TYPE action) { return children_[action]; }
...
void VNode::count(int c) { count_ = c; }
int VNode::count() const { return count_; }
void VNode::value(double v) { value_ = v; }
double VNode::value() const { return value_; }
...
void VNode::PrintTree(int depth, ostream& os) {
  ...
  os << "("
     << "d:" << this->default_move().value << " l:" << this->lower_bound()
     << ", u:" << this->upper_bound() << ", w:" << this->Weight()
     << ", weu:" << DESPOT::WEU(this) << ")" << endl;
  ...
  for (int a = 0; a < qnodes.size(); a++) {
    ...
    os << repeat("|   ", this->depth()) << "a=" << qnode->edge() << ": "
       << "(d:" << qnode->default_value << ", l:" << qnode->lower_bound()
       << ", u:" << qnode->upper_bound() << ", r:" << qnode->step_reward << ")"
       << endl;
...
QNode::QNode(int count, double value) : count_(count), value_(value) {}
...
map<OBS_TYPE, VNode*>& QNode::children() { return children_; }

VNode* QNode::Child(OBS_TYPE obs) { return children_[obs]; }
...
void QNode::count(int c) { count_ = c; }

int QNode::count() const { return count_; }

void QNode::value(double v) { value_ = v; }

double QNode::value() const { return value_; }
