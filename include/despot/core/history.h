...
class History {
 public:
  ...
  ACT_TYPE Action(int t) const { return actions_[t]; }

  OBS_TYPE Observation(int t) const { return observations_[t]; }
  ...
  ACT_TYPE LastAction() const { return actions_.back(); }

  OBS_TYPE LastObservation() const { return observations_.back(); }

  History Suffix(int s) const {
    History history;
    for (int i = s; i < Size(); i++) history.Add(Action(i), Observation(i));
    return history;
  }
...
struct less<despot::History> {
  bool operator()(const despot::History& h1, const despot::History& h2) const {
    ...
    for (int i = 0; i < N; i++) {
      if (h1.Action(i) < h2.Action(i)) return true;
      if (h1.Action(i) > h2.Action(i)) return false;
      if (h1.Observation(i) < h2.Observation(i)) return true;
      if (h1.Observation(i) > h2.Observation(i)) return false;
    }
    return false;
  }
};
