...
TigerState::TigerState() : tiger_position(0) {}
...
class OptimalTigerPolicy : public DefaultPolicy {
 public:
  OptimalTigerPolicy(const DSPOMDP* model, ParticleLowerBound* bound)
...
bool Tiger::Step(State& s, double random_num, ACT_TYPE action, double& reward,
  ...
  if (action == LEFT || action == RIGHT) {
    ...
    obs = state.tiger_position;  // can use arbitary observation
  } else {
    reward = -1;
    if (random_num <= 1.0 - NOISE)
...
ScenarioLowerBound* Tiger::CreateScenarioLowerBound(
    string name, string particle_bound_name) const {
  ScenarioLowerBound* bound = NULL;
  if (name == "TRIVIAL" || name == "DEFAULT") {
    bound = new TrivialParticleLowerBound(this);
  } else if (name == "RANDOM") {
    bound =
        new RandomPolicy(this, CreateParticleLowerBound(particle_bound_name));
  } else if (name == "LEFT") {
...
void Tiger::PrintBelief(const Belief& belief, ostream& out) const {
  out << "cur belief: " << belief.text() << endl;
}
...
int Tiger::NumActiveParticles() const { return memory_pool_.num_allocated(); }
